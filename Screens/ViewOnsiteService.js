...
const ViewOnsiteService = ({navigation, route}) => {
  ...
  const fetchData = async (fromDate, toDate, selectedValue) => {
    try {
      const response = await fetch(
        'https://devservice.adityagroup.com/aditya_service/rma/CPPartner/CISOnsiteServiceReqStatus/',
        {
          method: 'POST',
          headers: {
  ...
  const handleAdd = () => {
    navigation.navigate('OnsiteService', {
      username: username,
      token: token,
      userData: userData,
    });
  ...
  const handleModalOK = async () => {
    ...
    try {
      const response = await fetch(
        'https://devservice.adityagroup.com/aditya_service/rma/CPPartner/CISOnsiteServiceReqCancellation/',
        {
          method: 'POST',
          headers: {
            'Content-type': 'application/json',
...
export default ViewOnsiteService;
