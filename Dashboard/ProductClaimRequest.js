...
const ProductClaimRequest = ({navigation, route}) => {
  const {username, token,serialNumber} = route.params;

  const [inputValueSerial, setInputValueSerial] = useState('');
  ...
  const [warranty, setWarranty] = useState('WNC');

  useEffect(() => {
    if (serialNumber) {
      setInputValueSerial(serialNumber);
    }
  }, [serialNumber]);
  ...
  const handleScan = () =>{
    navigation.navigate('Scanner', {
      onScanComplete: (scannedSerialNumber) => {
        setInputValueSerial(scannedSerialNumber);
      },
    });
  }
    ...
    <ScrollView>
      <View style={styles.container}>
        ...
        <View style={styles.inputContainer}>
          <TextInput
            style={styles.input1}
            value={inputValueSerial}
            onChangeText={text => setInputValueSerial(text)}
          />
...
export default ProductClaimRequest;
